// ============================================================
// GEOCODE CIUDADES — consulta la BD, geocodifica las ciudades
// que faltan en ciudades-coords.json y actualiza el fichero.
//
// Uso: node geocode-ciudades.cjs
...
const COORDS_FILE                = path.join(__dirname, 'ciudades-coords.json');
const NOMINATIM_DELAY_MS         = 2000;
const NOMINATIM_INTENTO_DELAY_MS = 2500;
      ...
      res.on('end', () => {
        try {
          ...
          if (r.features && r.features.length > 0) {
            ...
            resolve({ lat: parseFloat(lat.toFixed(7)), lng: parseFloat(lng.toFixed(7)) });
          } else {
            resolve(null);
          }
        } catch { resolve(null); }
...
// 1. ciudad + Cataluña         → más preciso
// 2. ciudad + España           → para ciudades fuera de Cataluña
// 3. solo ciudad               → último recurso
async function geocodificar(nombreCiudad) {
  const queries = [
  ...
  for (let i = 0; i < queries.length; i++) {
    ...
    await sleep(500);
    const r2 = await nominatimGet(queries[i]);
...
async function main() {
  ...
  `);
  await client.end();
  console.log(`Ciudades en BD: ${rows.length}`);

  // 3. Filtrar pendientes (las que no están en el JSON)
  const pendientes = rows.filter(r => !coords[r.slug]);
  ...
  for (let i = 0; i < pendientes.length; i++) {
    ...
    process.stdout.write(`[${i + 1}/${pendientes.length}] ${r.slug} → `);

    const { coords: resultado, intento } = await geocodificar(r.nombre);

    if (resultado) {
      coords[r.slug] = resultado;
      ...
      fallidos++;
      fallidos_lista.push(r.slug);
    }

    // Guardar cada 20 ciudades por si se interrumpe
    if ((i + 1) % 20 === 0) {
  ...
  const ordenado = Object.fromEntries(
    Object.entries(coords).sort(([a], [b]) => a.localeCompare(b))
  );
  ...
  if (fallidos_lista.length > 0) {
    const logPath = path.join(__dirname, 'geocode-ciudades-fallidos.txt');
    fs.writeFileSync(logPath, fallidos_lista.join('\n'), 'utf8');
    console.log(`\nCiudades no encontradas → ${logPath}`);
  }
}
